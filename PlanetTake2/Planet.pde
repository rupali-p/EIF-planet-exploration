class Planet {
  ...
  void CreatePlanetMain(FloatList colour, FloatList shape, float phase, float avgTemp, int currentDay) {
    push();
      rotateX(phase/2);
    rotateY(phase/2);
    rotateZ(phase/2);
    for (int i = 0; i < total + 1; i++) {
      ...
      for (int j = 0; j < total + 1; j++) {
        ...
        if (i!=0) {
          ...
          if (i < total) {
            float thisTemp = colour.get(currentDay);
